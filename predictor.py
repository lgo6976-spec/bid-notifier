...
# -*- coding: utf-8 -*-
"""
과거 낙찰 이력(model_data.json, 포항 지역 축산 관련 262건 실낙찰 데이터) 기반
낙찰 확률/추천 입찰가 예측 모듈.
...
※ 예정가격 추첨 불확실성에 대해 (2026-09-22 재검증):
  ...
  37~49%가 "예정가격 최저가 미만 무효"로 튕겨나가고, 진짜 성공률은 14~22%
...
버퍼 없음 + 학교 자체 평균(A) 조합으로 262건 leave-one-out 백테스트를
재검증한 "실제"(원화 금액 환산 + 그 날 진짜 예정가격 대입 + 무효 여부까지
반영) 성공률이다(실제 predict() 함수를 그대로 돌려서 확인한 수치):
  공격적(내부목표50%) -> 실제성공률 14.1%  (무효율 37.0%)
  표준(내부목표70%)   -> 실제성공률 19.5%  (무효율 45.0%)
  보수적(내부목표85%) -> 실제성공률 21.4%  (무효율 48.1%)
  최대안전(내부목표95%) -> 실제성공률 21.0% (무효율 49.2%)

  (2026-09-22 추가: 91개 학교 전체를 대상으로 재조회해보니, 품목명이 명확한
  확정낙찰 건 중 model_data.json에 없던 145건을 발견 - 기존 수집이 91개교
  전체를 훑은 게 아니라 일부만 커버했던 것으로 보임. 그 중 88건을 성공적으로
  조회해 채워넣어 174->262건으로 보강함, 38개 학교로 확장. 나머지 57건은
  실제로는 낙찰 미확정(유찰/취소 등)이라 조회 결과 None을 반환해 제외함.)
※ 무효율이 여전히 36~45%로 높다는 걸 이메일/대시보드에서 사람이 바로 알 수
...
# (label, 내부목표percentile, 실제성공률%, 실제무효율%) - 262건 leave-one-out 검증값
