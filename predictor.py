...
- "마진"(=학교별 목표 여유폭, 최저가 위로 몇 %p를 더 얹어 쓸지)을 예전에는
  ...
  구도를 이미 암묵적으로 반영한다.
- 학교 자체 이력이 5건 미만이면(소표본이라 여유폭 탐색이 못 미더우므로)
  전체 174건 풀로 대체해서 여유폭을 찾는다. 예정가격비율(mean_ratio) 자체는
  1건이라도 있으면 그 학교 값을 그대로 신뢰한다(A/B/C 세 방식 비교에서
  검증됨 - 표본이 적어도 학교 고유 신호가 있어서 전체평균보다 나음).
...
함수를 그대로 돌려서 확인한 수치):
  공격적   -> 실제성공률 28.2%  (무효율 44.3%)
  표준     -> 실제성공률 14.9%  (무효율 13.8%)
  보수적   -> 실제성공률 16.1%  (무효율  7.5%)
  최대안전 -> 실제성공률 12.6%  (무효율  5.2%)
※ 이전(전체 공통 percentile) 방식 대비 무효율이 크게 낮아졌다(최대안전 기준
  48.9% -> 5.2%). 승률은 등급별로 비슷하거나 소폭 낮아졌지만, 무효(자격
...
ROUND_UNIT = 100
MIN_RECORDS = 5  # 이 미만이면 여유폭 탐색은 전체 풀로 대체(예정가격비율 추정은 별개)
OFFSET_MAX = 3.0  # 여유폭 탐색 범위 상한(%p)
OFFSET_STEP = 0.02

# (label, 허용 무효율 상한, 실제성공률%, 실제무효율%) - 174건 leave-one-out 검증값
TIERS = [
    ("공격적", 1.01, 28.2, 44.3),   # 사실상 무제한 - 승률만 최대화
    ("표준", 0.20, 14.9, 13.8),
    ("보수적", 0.05, 16.1, 7.5),
    ("최대안전", 0.0, 12.6, 5.2),
]
